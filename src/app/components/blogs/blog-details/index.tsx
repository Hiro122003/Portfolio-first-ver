...
export default function BlogDetailHome({ blogData }: { blogData: Blog }) {
  ...
  async function handleCommentSave() {
    ...
    // console.log(comments,'comments') //commentsは空の配列を取得
    extractComments.push(`${comment}|${session?.user?.name}`);
    // console.log(comments) //['test comments|Some_Username_145812649']
            ...
            {session !== null ? (
              <>
                <input
                  ...
                  className="w-full rounded-md border border-transparent py-3 px-6 text-base text-body-color placeholder-body-color shadow-one outline-none focus:border-primary dark:bg-[#242B51] focus-visible:shadow-none  dark:shadow-signUp"
                />
